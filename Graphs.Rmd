...
```{r Cambiamento del prezzo di ETH}
...
```{r Andamento del mercato della crypto arte, fig.width=5}
...
```
```{r}
ggsave("figure/mercato_cryptoarte.png", width = 9, height = 6)
```

```{r Dettaglio andamento del mercato della crypto arte}
crypto_art %>%
  ...
  ggtitle("Dettaglio volume di vendita del mercato della crypto art", "Periodo Gennaio 21 - Maggio 21") +
  xlab("Anni") +
  ylab("Dollari [USD]") +
  scale_y_continuous(labels = function(v) ifelse(v == "0", paste0("$", v / 1000000), paste0("$", v / 1000000,"M"))) +
...
```
```{r}
ggsave("figure/dettaglio_mercato_cryptoarte.png", width = 7, height = 4)
```
...
```
```{r}
ggsave("figure/distribuzione_cryptoarte.png", width = 9, height = 10)
```

```{r SuperRare Andamento vendite}
...
```
```{r}
ggsave("figure/vendite_superrare.png", width = 8, height = 5)
```

```{r Dettaglio SuperRare andamento vendite}
freq_d <- sales %>%
  mutate(month = format(timestamp, "%m"), year = format(timestamp, "%Y")) %>%
  ...
  ggplot(aes(date, count)) +
  geom_col(fill = "#88A2BC") +
  geom_label(data = ~.x %>% filter(date %in% c(as.Date("2020-04-01"), as.Date("2021-01-01"))), aes(label = count), vjust = 0) +
  scale_x_date(date_breaks = "2 month", date_labels = "%b %Y") +
  ggtitle("Dettaglio andamento delle vendite di crypto art su SuperRare", "Frequenza mensile delle vendite") +
  xlab("") +
  ylab("Unità vendute") +
  theme_bw()
...
tot_d <- sales %>%
  ...
  geom_line(color = "#88A2BC") +
  geom_point(color = "#88A2BC") +
  geom_label(data = ~.x %>% filter(date %in% c(as.Date("2020-04-01"), as.Date("2021-01-01"))), aes(label = ifelse(total < 1e6, paste0("$", round(total / 1e3, 1), "k"), paste0("$", round(total / 1e6, 1), "M"))), vjust = 0, hjust = 0) +
  scale_y_continuous(breaks = scales::pretty_breaks(), labels = function(v) ifelse(v == 0, paste0("$", 0), paste0("$", v / 1000000, "M"))) +
...
```
```{r}
ggsave("figure/dettaglio_vendite_superrare.png", width = 8, height = 5)
```

```{r SuperRare Primario vs Secondario}
...
```
```{r}
ggsave("figure/primario_vs_secondario.png", width = 7, height = 4)
```
